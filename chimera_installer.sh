...
mount_partitions() {
    ...
    if [ "$(ls -A "$INSTALL_ROOT/boot" 2>/dev/null | grep -v '^EFI\|System Volume Information')" ]; then
...
configure_system() {
    ...
    if [ -n "$username" ]; then
        ...
        chimera-chroot "$INSTALL_ROOT" /bin/sh -c "
            useradd -m -s /bin/sh '$username' || useradd -m -s /bin/sh '$username'
            groupadd wheel 2>/dev/null || groupmod wheel 2>/dev/null || true
            groupadd audio 2>/dev/null || groupmod audio 2>/dev/null || true
            groupadd video 2>/dev/null || groupmod video 2>/dev/null || true
            groupadd netdev 2>/dev/null || groupmod netdev 2>/dev/null || true
            usermod -aG wheel,audio,video,netdev '$username' 2>/dev/null || true
        "
