...
# Supports /dev/sdX, /dev/emmcX and /dev/nvmeXn1 drives
...
show_main_menu() {
    show_header
    printf "${WHITE}WYBIERZ TRYB PRACY:${NC}\n"
    printf "\n"
    printf "1) ${GREEN}Instalacja systemu${NC} - kompletna instalacja Chimera Linux\n"
    printf "2) ${YELLOW}Naprawa systemu${NC} - diagnoza i naprawa zainstalowanego systemu\n"
    printf "3) ${RED}Wyjście${NC}\n"
    printf "\n"
    
    while true; do
        printf "Wybierz tryb (1-3): "
        read choice
        case "$choice" in
            1) return 1;;  # Installation mode
            2) return 2;;  # Repair mode  
            3) printf "${YELLOW}Wyjście z programu${NC}\n"; exit 0;;
            *) printf "${RED}Nieprawidłowy wybór. Wybierz 1, 2 lub 3.${NC}\n";;
        esac
    done
}
...
show_requirements() {
    ...
    printf "  • Flaga ESP jest obowiązkowa dla UEFI\n"
    printf "\n"
...
# Check if disk is used by live system
check_live_system_usage() {
    local disk="$1"
    if findmnt -n -o SOURCE | grep -q "^$disk"; then
        printf "${RED}UWAGA: Wybrany dysk jest używany przez system live!${NC}\n"
        printf "${YELLOW}Nie można użyć tego dysku do instalacji.${NC}\n"
        return 1
    fi
    return 0
}
...
select_disk() {
    ...
    while IFS='|' read -r disk type size model; do
        printf "%2d) %-12s %-10s %-8s %s" "$i" "$disk" "$type" "$size" "$model"
        
        # Check if disk is used by live system
        if findmnt -n -o SOURCE | grep -q "^$disk"; then
            printf " ${RED}[LIVE SYSTEM]${NC}"
        fi
        printf "\n"
        i=$((i + 1))
    done < "$DISK_LIST_FILE"
    ...
    while true; do
        ...
        if [ "$choice" -ge 1 ] && [ "$choice" -le "$total_disks" ]; then
            ...
            # Check if selected disk is used by live system
            if ! check_live_system_usage "$SELECTED_DISK"; then
                read -p "Naciśnij Enter aby wybrać inny dysk..." dummy
                continue
            fi
...
check_suitable_layout() {
        ...
        if [ -b "$part" ]; then
            ...
            case "$part_type" in
                c12a7328*)
                    ...
                    esp_flag=$(parted "$SELECTED_DISK" print | grep "^ $part_num " | awk '{print $6}')
                    if echo "$esp_flag" | grep -q "esp"; then
                        efi_flag_ok=true
                    fi
                    break
                    ;;
    ...
    # Check for suitable root partition
    for part in "${part_prefix}"*; do
            ...
            case "$part_type" in
                0fc63daf*)
                    if [ "$part_size_gb" -ge 14 ]; then
                        ...
                        # Check boot flag
                        part_num=$(echo "$part" | grep -o '[0-9]*$')
                        boot_flag=$(parted "$SELECTED_DISK" print | grep "^ $part_num " | awk '{print $6}')
                        if echo "$boot_flag" | grep -q "boot"; then
                            root_flag_ok=true
                        fi
                        break
                    fi
    ...
    if [ "$efi_found" = true ] && [ "$root_found" = true ] && [ "$efi_size" -ge 200 ] && [ "$efi_flag_ok" = true ] && [ "$root_flag_ok" = true ]; then
        printf "${GREEN}Znaleziono odpowiedni układ partycji:${NC}\n"
        printf "  EFI: %s (%dMB) [flagi: %s]\n" "$EFI_PARTITION" "$efi_size" "$(parted "$SELECTED_DISK" print | grep "^ 1 " | awk '{print $6}')"
        printf "  Root: %s (%dGB) [flagi: %s]\n" "$ROOT_PARTITION" "$root_size" "$(parted "$SELECTED_DISK" print | grep "^ 2 " | awk '{print $6}')"
        printf "\n"
        return 0
    else
        ...
        elif [ "$efi_flag_ok" != true ]; then
            printf "  • Partycja EFI nie ma flagi ESP\n"
        fi
        if [ "$root_found" != true ]; then
            printf "  • Brak odpowiedniej partycji root (>14GB)\n"
        elif [ "$root_flag_ok" != true ]; then
            printf "  • Partycja root nie ma flagi boot\n"
        fi
        printf "\n"
        return 1
    fi
...
create_partitions() {
    ...
    parted -s "$SELECTED_DISK" set 1 esp on      # KLUCZOWA: Flaga ESP
    parted -s "$SELECTED_DISK" set 1 boot on     # Dodatkowa flaga boot
    
    # Create root partition (remaining space)
    printf "${BLUE}Tworzenie partycji root...${NC}\n"
    parted -s "$SELECTED_DISK" mkpart primary 513MiB 100%
    parted -s "$SELECTED_DISK" set 2 boot on    # KLUCZOWA: Flaga boot dla root
    ...
    printf "${BLUE}Weryfikacja flag partycji...${NC}\n"
    ESP_FLAG=$(parted "$SELECTED_DISK" print | grep "^ 1 " | awk '{print $6}')
    ROOT_FLAG=$(parted "$SELECTED_DISK" print | grep "^ 2 " | awk '{print $6}')
    
    if [ "$ESP_FLAG" != "esp,boot" ]; then
        printf "${YELLOW}Ostrzeżenie: Flaga ESP nie jest poprawnie ustawiona${NC}\n"
        parted -s "$SELECTED_DISK" set 1 esp on
        parted -s "$SELECTED_DISK" set 1 boot on
    fi
    
    if [ "$ROOT_FLAG" != "boot" ]; then
        printf "${YELLOW}Ostrzeżenie: Flaga boot nie jest poprawnie ustawiona${NC}\n"
...
mount_partitions() {
    ...
    # Check if filesystem is not empty
    ls_result=$(ls -A "$INSTALL_ROOT" 2>/dev/null)
    if [ -n "$ls_result" ]; then
        printf "${YELLOW}System plików nie jest pusty. Zawiera:${NC}\n"
        ls -la "$INSTALL_ROOT"
        printf "\n${WHITE}Opcje:${NC}\n"
        printf "1) Wyczyść zamontowany system plików\n"
        printf "2) Sformatuj partycje ponownie (usunie wszystkie dane)\n"
        printf "3) Anuluj instalację\n"
        printf "\n"
        ...
        while true; do
            ...
            case "$choice" in
                1)
                    printf "${BLUE}Czyszczenie zamontowanego systemu plików...${NC}\n"
                    ...
                    sync
                    printf "${GREEN}System plików wyczyszczony${NC}\n"
                    break
                    ;;
                2)
                    printf "${RED}Ponowne formatowanie partycji...${NC}\n"
                    umount "$INSTALL_ROOT" 2>/dev/null || true
                    
                    # Reformat root partition
                    printf "${BLUE}Formatowanie partycji root jako f2fs...${NC}\n"
                    mkfs.f2fs -f -l "chimera-root" "$ROOT_PARTITION"
                    ...
                    mount "$ROOT_PARTITION" "$INSTALL_ROOT"
                    chmod 755 "$INSTALL_ROOT"
                    printf "${GREEN}Partycje przeformatowane${NC}\n"
                    break
                    ;;
                3)
                    printf "${YELLOW}Instalacja anulowana${NC}\n"
                    exit 0
                    ;;
                *)
                    printf "${RED}Nieprawidłowy wybór. Wybierz 1, 2 lub 3.${NC}\n"
    ...
    # Set secure permissions for /boot directory (only root access)
...
install_system() {
    ...
    # Check if directory is clean after mounting
    ls_check=$(ls -A "$INSTALL_ROOT" 2>/dev/null | grep -v '^boot$' || true)
    if [ -n "$ls_check" ]; then
        printf "${YELLOW}Katalog instalacji nadal zawiera pliki. Używanie flagi -f...${NC}\n"
        FORCE_FLAG="-f"
    else
        printf "${GREEN}Katalog instalacji jest czysty${NC}\n"
        FORCE_FLAG=""
    fi
...
configure_system() {
    ...
    printf "${BLUE}Generowanie /etc/fstab...${NC}\n"
    {
        ...
        printf "# Note: EFI System Partition is mounted at /boot for systemd-boot\n"
        printf "\n"
        ...
        # Add EFI partition with secure mount options
        EFI_UUID=$(lsblk -no UUID "$EFI_PARTITION" 2>/dev/null || echo "")
        if [ -n "$EFI_UUID" ]; then
            printf "UUID=%s /boot vfat defaults,umask=0077,fmask=0177,dmask=0077 0 2\n" "$EFI_UUID"
        else
            printf "%s /boot vfat defaults,umask=0077,fmask=0177,dmask=0077 0 2\n" "$EFI_PARTITION"
        fi
    ...
    if [ -n "$username" ]; then
        ...
        # Create user using useradd (official Chimera way)
        ...
        # Ask about adding to wheel group (sudo access)
        printf "\n${CYAN}Czy chcesz dodać użytkownika do grupy wheel (dostęp sudo)? (t/N): ${NC}"
        read add_wheel
        case "$add_wheel" in
            [Tt]*)
                printf "${BLUE}Dodawanie do grupy wheel...${NC}\n"
                chimera-chroot "$INSTALL_ROOT" /bin/sh -c "usermod -a -G wheel '$username'"
                printf "${GREEN}Użytkownik dodany do grupy wheel${NC}\n"
                ;;
            *)
                printf "${YELLOW}Użytkownik nie będzie miał dostępu sudo${NC}\n"
                ;;
        esac
        
        # Ask about other groups
        printf "\n${CYAN}Czy chcesz dodać użytkownika do dodatkowych grup?${NC}\n"
        printf "Dostępne grupy: audio, video, kvm, plugdev\n"
        printf "Podaj grupy oddzielone przecinkami (lub Enter aby pominąć): "
        read extra_groups
        
        if [ -n "$extra_groups" ]; then
            printf "${BLUE}Dodawanie do grup: %s${NC}\n" "$extra_groups"
            chimera-chroot "$INSTALL_ROOT" /bin/sh -c "usermod -a -G '$extra_groups' '$username'"
        fi
    ...
    printf "${BLUE}Podstawowa konfiguracja systemu...${NC}\n"
    
    # Set hostname
    printf "Podaj nazwę hosta (hostname): "
    read hostname
    if [ -n "$hostname" ]; then
        printf "${BLUE}Ustawianie hostname: %s${NC}\n" "$hostname"
        echo "$hostname" > "$INSTALL_ROOT/etc/hostname"
    fi
    
    # Set timezone
    printf "\n${CYAN}Dostępne strefy czasowe (przykłady):${NC}\n"
    printf "  Europe/Warsaw, Europe/Berlin, US/Eastern, US/Pacific\n"
    printf "  Asia/Tokyo, Australia/Sydney, UTC\n"
    printf "Podaj strefę czasową (lub Enter dla UTC): "
    read timezone
    ...
    printf "${BLUE}Ustawianie strefy czasowej: %s${NC}\n" "$timezone"
    if [ -f "$INSTALL_ROOT/usr/share/zoneinfo/$timezone" ]; then
        chimera-chroot "$INSTALL_ROOT" /bin/sh -c "ln -sf /usr/share/zoneinfo/$timezone /etc/localtime"
    else
        printf "${YELLOW}Nie znaleziono strefy czasowej, używam UTC${NC}\n"
        chimera-chroot "$INSTALL_ROOT" /bin/sh -c "ln -sf /usr/share/zoneinfo/UTC /etc/localtime"
    fi
    
    # Enable syslog
    printf "${BLUE}Włączanie systemu logowania...${NC}\n"
    chimera-chroot "$INSTALL_ROOT" /bin/sh -c "dinitctl enable syslog-ng" || true
...
install_bootloader() {
    ...
    # Check if ESP is mounted as /boot
    if ! mountpoint -q "$INSTALL_ROOT/boot"; then
        printf "${RED}Błąd: Partycja EFI nie jest zamontowana jako /boot${NC}\n"
        printf "${YELLOW}Montowanie partycji EFI jako /boot...${NC}\n"
        mkdir -p "$INSTALL_ROOT/boot"
        mount "$EFI_PARTITION" "$INSTALL_ROOT/boot"
    fi
    
    # Verify ESP is FAT32 and mounted at /boot
    if ! findmnt -n -o FSTYPE -T "$INSTALL_ROOT/boot" | grep -q "vfat"; then
        printf "${RED}Błąd: Partycja /boot nie jest systemem plików FAT32${NC}\n"
        printf "${YELLOW}Formatowanie partycji EFI jako FAT32...${NC}\n"
        umount "$INSTALL_ROOT/boot" 2>/dev/null || true
        mkfs.fat -F32 -n "EFI" "$EFI_PARTITION"
        mount "$EFI_PARTITION" "$INSTALL_ROOT/boot"
    fi
    ...
    # Create random seed file to avoid warnings
    printf "${BLUE}Tworzenie pliku random seed...${NC}\n"
    chimera-chroot "$INSTALL_ROOT" /bin/sh -c "
        mkdir -p /boot/loader
        dd if=/dev/urandom of=/boot/loader/random-seed bs=512 count=1 2>/dev/null
        chmod 600 /boot/loader/random-seed
    "
    
    # Install bootloader with explicit path
    printf "${BLUE}Instalacja bootloadera...${NC}\n"
    chimera-chroot "$INSTALL_ROOT" /bin/sh -c "bootctl install --path=/boot"
...
EOF
    ...
    # Set secure permissions for loader config
    chmod 600 "$INSTALL_ROOT/boot/loader/loader.conf"
    
    # Generate boot entries using Chimera's generator
    ...
    printf "${BLUE}Weryfikacja instalacji systemd-boot...${NC}\n"
    if [ -f "$INSTALL_ROOT/boot/EFI/BOOT/BOOTX64.EFI" ] && [ -f "$INSTALL_ROOT/boot/loader/loader.conf" ]; then
        printf "${GREEN}systemd-boot zainstalowany poprawnie${NC}\n"
    else
        printf "${RED}Błąd: systemd-boot nie został poprawnie zainstalowany${NC}\n"
    fi
    
    printf "${GREEN}systemd-boot zainstalowany i skonfigurowany${NC}\n"
    printf "${GREEN}Bezpieczne uprawnienia ustawione dla /boot${NC}\n"
}
...
show_summary() {
    ...
    printf "  • ESP jako /boot: TAK (wymagane dla systemd-boot)\n"
    printf "\n"
    ...
    printf "\n"
    printf "${CYAN}Dalsze konfiguracje po pierwszym uruchomieniu:${NC}\n"
    printf "  • Zarządzanie pakietami: 'apk add', 'apk search'\n"
    printf "  • Konfiguracja klawiatury: /etc/default/keyboard\n"
    printf "  • Konfiguracja konsoli: /etc/default/console-setup\n"
    printf "  • Zegar sprzętowy: echo 'localtime' > /etc/hwclock (dla dual-boot)\n"
    printf "  • Logi systemu w: /var/log/messages\n"
    printf "\n"
    printf "${CYAN}Ważne informacje:${NC}\n"
    printf "  • ESP partycja jest zamontowana jako /boot (wymagane dla systemd-boot)\n"
    printf "  • Flaga ESP jest obowiązkowa dla UEFI firmware\n"
    printf "  • Flaga boot dla partycji root zapewnia zgodność\n"
    printf "  • Flaga -f użyta zgodnie z dokumentacją dla istniejących partycji\n"
    printf "  • gen-systemd-boot automatycznie aktualizuje wpisy przy instalacji kerneli\n"
    printf "\n"
    printf "${CYAN}Dokumentacja konfiguracji:${NC}\n"
    printf "  https://chimera-linux.org/docs/installation/install\n"
...
repair_mode() {
    show_header
    printf "${WHITE}TRYB NAPRAWY SYSTEMU${NC}\n"
    printf "\n"
    printf "${YELLOW}Funkcja naprawy jest w przygotowaniu${NC}\n"
    printf "\n"
    printf "Naciśnij Enter aby powrócić do menu głównego..."
    read dummy
}
